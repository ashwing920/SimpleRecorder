...
static int GetStreamParam()
{
    static struct v4l2_streamparm parm={0};
    int ret;
 
    parm.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    if ((ret = ioctl(fd_cam, VIDIOC_G_PARM, &parm)) < 0) {
        perror("Error ioctl VIDIOC_G_PARM");
        return 0;
		}
  	printf("capture.denominator:%d\n",parm.parm.capture.timeperframe.denominator);
	printf("capture.numerator:%d\n",parm.parm.capture.timeperframe.numerator);
    parm.parm.capture.timeperframe.denominator = 50;
    parm.parm.capture.timeperframe.numerator = 1;
	//parm.parm.capture.capturemode = V4L2_MODE_HIGHQUALITY;
	//parm.parm.capture.capability = V4L2_CAP_TIMEPERFRAME;
    if ((ret = ioctl(fd_cam, VIDIOC_S_PARM, &parm)) < 0)
...
{	
	v4l2_std_id std_id;
	std_id=2048;
	if (-1 == ioctl (fd_cam, VIDIOC_S_STD, &std_id)) {
		perror ("VIDIOC_S_STD");
		return 0;
		}
	return 1;
...
int camera_get_frame(struct picture_t *pic)
{
	...
	applog("buf_index=%02d, seq=%d, timestamp=%d|%6d", 
